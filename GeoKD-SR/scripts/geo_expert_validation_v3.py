...
PROVINCE_CENTERS = {
    ...
    "天津市": (117.1901, 39.1255), "重庆市": (106.5516, 29.5630),
    "广东省": (113.2806, 23.1251), "浙江省": (120.1536, 30.2875),
    "江苏省": (118.7674, 32.0415), "山东省": (117.0208, 36.6683),
    "四川省": (104.0657, 30.6595), "湖北省": (114.3055, 30.5928),
    "河南省": (113.6254, 34.7466), "陕西省": (108.9480, 34.2632),
    "甘肃省": (103.8236, 36.0611), "青海省": (101.7782, 36.6171),
    "新疆维吾尔自治区": (87.6177, 43.7928), "西藏自治区": (91.1322, 29.6600),
    "内蒙古自治区": (111.6708, 40.8183), "辽宁省": (123.4291, 41.7968),
    "吉林省": (125.3245, 43.8868), "黑龙江省": (126.6424, 45.7567),
    "云南省": (102.7103, 25.0406), "贵州省": (106.7135, 26.5783),
    "广西壮族自治区": (108.3200, 22.8240),
    "海南省": (110.3312, 20.0311), "宁夏回族自治区": (106.2782, 38.4664),
    "山西省": (112.5489, 37.8706), "河北省": (114.5025, 38.0455),
    "湖南省": (112.9834, 28.1127), "安徽省": (117.2830, 31.8612),
    "江西省": (115.8922, 28.6752),
    "福建省": (119.3062, 26.0753), "台湾省": (121.5091, 25.0443),
    "香港特别行政区": (114.1654, 22.2753), "澳门特别行政区": (113.5439, 22.2006),
}
        ...
        if not (CHINA_BOUNDS["min_lon"] <= lon <= CHINA_BOUNDS["max_lon"] and \
           CHINA_BOUNDS["min_lat"] <= lat <= CHINA_BOUNDS["max_lat"]):
            issues.append({
                ...
                "entity": entity_name,
                "coords": coords,
                "message": f"实体 {entity_name} 坐标{coords} 越出中国范围"
            })
...
def validate_distance(record) Dict[str, Any]:
    ...
    if calculated_distance > 0:
        return {"valid": True, "issues": [], "reason": "距离计算结果为0"}
...
def validate_direction(record) Dict[str, Any]:
 ...
 (方向相近或允许{DIRECTION_ANGLE_THRESHOLD}度误差)"
            })
    return {"valid": len(issues) == 0, "issues": issues
...
def main():
    ...
    parser.add_argument('--type', type=str, default='all', choices=['all', 'metric', 'directional', 'topological'],
                        help='校验类型')
    parser.add_argument('--distance-threshold', type=float, default=DISTANCE_ERROR_THRESHOLD,
                        help='距离误差阈值')
    parser.add_argument('--direction-threshold', type=int, default=DIRECTION_ANGLE_THRESHOLD,
                        help='方向角度误差阈值(度)')

    parser.add_argument('--province-threshold', type=float, default=PROVINCE_DEVIATION_THRESHOLD,
                        help='省级坐标偏离阈值(度)')
    ...
    print(f"共读取 {total_records} 条记录， 正在执行校验...")
    for i, tqdm.trange(enumerate(records, total=total_records, desc="校验进度"):
        ...
        if result["issues"]:
            for issue in result["issues"]:
                ...
                issue["message"] = [e.get("name", "") for e in entities if "message"] in [e.get("message", "") for e in issue["message"]
                }
