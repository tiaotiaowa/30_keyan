...
MODEL_NAME = "glm-4.7"  # 使用GLM-4-Plus模型
...
CONTAINS_BATCH_PROMPT = """【任务】一次生成5条中国地理拓扑包含关系推理问题
...
【输出格式】严格输出JSON数组，包含5条记录，每条记录格式如下：
[
{{
  "id": "geosr_topological_{{序号:05d}}",
  "spatial_relation_type": "topological",
  ...
  "answer": "简短明确答案（5-30字符）",
  "reasoning_chain": [
    {{"step": 1, "name": "entity_identification", "action": "extract_entities", "content": "详细识别实体信息（50-100字符，包含地理位置、行政级别、地理特征等）", "entities_involved": ["实体1", "实体2"]}},
    {{"step": 2, "name": "spatial_relation_extraction", "action": "classify_relation", "content": "分析空间关系类型（40-80字符，解释为什么是拓扑关系）", "relation_type": "topological"}},
    {{"step": 3, "name": "coordinate_retrieval", "action": "infer_entity_to_token", "content": "获取坐标并解释地理意义（60-100字符，包含经纬度、相对位置、地理分区等）", "coordinates": "根据实际坐标填写"}},
    {{"step": 4, "name": "spatial_calculation", "action": "determine_topology", "content": "详细分析包含关系（60-100字符，包含行政隶属、空间范围、边界关系等）", "calculation_result": "contains"}},
    {{"step": 5, "name": "answer_generation", "action": "generate_answer", "content": "生成简短答案（30-50字符）", "final_answer": "简短直接答案"}}
  ...
  "entities": [
    {{"name": "实体1", "type": "实体1_type", "coords": "[经度, 纬度]"}},
    {{"name": "实体2", "type": "实体2_type", "coords": "[经度, 纬度]"}}
  ],
  "spatial_tokens": ["实体1", "实体2", "包含", "位于", "其他关键词"]（由问题中的空间关键词决定）,
  "entity_to_token": "根据question中实体位置填写char_start/char_end/token_indices",
}},
  ... （共5条记录）
]
...
现在请根据上述5对实体生成5条contains类型的数据，输出JSON数组："""

OVERLAP_BATCH_PROMPT = """【任务】一次生成5条中国地理拓扑交叉关系推理问题
...
[
  {{
    "id": "geosr_topological_{{序号1:05d}}",
    "spatial_relation_type": "topological",
    "topology_subtype": "overlap",
    "question": "生成的问题文本（15-80字符）",
    "answer": "简短明确答案（5-30字符）",
    "reasoning_chain": [
      {{"step": 1, "name": "entity_identification", "action": "extract_entities", "content": "详细识别实体信息（50-100字符）", "entities_involved": ["实体1", "实体2"]}},
      {{"step": 2, "name": "spatial_relation_extraction", "action": "classify_relation", "content": "分析空间关系类型（40-80字符）", "relation_type": "topological"}},
      {{"step": 3, "name": "coordinate_retrieval", "action": "infer_entity_to_token", "content": "获取坐标并解释地理意义（60-100字符）", "coordinates": {{"实体1": "坐标1", "实体2": "坐标2"}}}},
      {{"step": 4, "name": "spatial_calculation", "action": "determine_topology", "content": "详细分析交叉关系（60-100字符）", "calculation_result": "overlap"}},
      {{"step": 5, "name": "answer_generation", "action": "generate_answer", "content": "生成简短答案（30-50字符）", "final_answer": "简短直接答案"}}
    ],
    "entities": [
      {{"name": "实体1名称", "type": "实体1类型", "coords": [经度, 纬度]}},
      {{"name": "实体2名称", "type": "实体2类型", "coords": [经度, 纬度]}}
    ],
    "spatial_tokens": ["实体1", "实体2", "流经", "其他关键词"]（问题中出现的空间关键词）,
    "entity_to_token": {{"实体1": {{"char_start": X, "char_end": Y, "token_indices": [X, Y]}}, "实体2": {{"char_start": A, "char_end": B, "token_indices": [A, B]}}}},
    "difficulty": "easy/medium/hard",
    "difficulty_score": "根据难度填写2.0-4.5"
  }},
  ... （共5条记录）
]
...
5. 确保所有坐标在中国境内
6. entity_to_token的位置必须与question中的实际位置匹配
7.必须注意overlap不是contains，如果两个实体间没有各自独立的区域则不属于overlap关系，需要重新为我提供符合overlap关系的实体


现在请根据上述5对实体生成5条overlap类型的数据，输出JSON数组："""

def format_pairs_for_batch(pairs: List[Dict], subtype: str) -> str:
    """格式化实体对列表用于批量提示词"""
    lines = []
    for i, pair in enumerate(pairs):
        if subtype == 'contains':
            ...
            lines.append(
                f"  {i+1}. 容器实体：{container['name']}（类型：{container['type']}，坐标：{container['coords']}）\n"
                f"     被包含实体：{contained['name']}（类型：{contained['type']}，坐标：{contained['coords']}）\n"
                f"     难度：{difficulty}"
            )
        else:  # overlap
            ...
            difficulty = pair.get('difficulty', 'medium')
            lines.append(
                f"  {i+1}. 实体1：{entity1['name']}（类型：{entity1['type']}，坐标：{entity1['coords']}）\n"
                f"     实体2：{entity2['name']}（类型：{entity2['type']}，坐标：{entity2['coords']}）\n"
                f"     交叉信息：{overlap_info}\n"
                f"     难度：{difficulty}"
            )
    return '\n'.join(lines)
...
def build_batch_prompt(pairs: List[Dict], subtype: str, start_seq: int) -> str:
    """构建批量提示词（5条数据）"""
    entity_pairs_list = format_pairs_for_batch(pairs, subtype)

    if subtype == 'contains':
        return CONTAINS_BATCH_PROMPT.format(
            entity_pairs_list=entity_pairs_list,
            start_seq=start_seq
        )
    else:
        return OVERLAP_BATCH_PROMPT.format(
            entity_pairs_list=entity_pairs_list,
            start_seq=start_seq
        )


def parse_batch_json(response: str) -> List[Dict]:
    """解析批量JSON响应（期望5条记录的数组）"""
    if not response:
        return []
        ...
        for attempt in range(max_retries):
            try:
                response = self.client.chat.completions.create(
                    ...
                    temperature=0.7,
                    max_tokens=16000  # 增加到8192以支持5条完整记录
                )
...
def main():
    ...
    # 采样（使用之前计算的target_count）
    if args.test:
        target_count = 5  # 测试模式固定5条
    ...
    with open(OUTPUT_FILE, 'a', encoding='utf-8') as out_f:
        # 批量处理
        total_batches = (len(sampled_pairs) + batch_size - 1) // batch_size

        for batch_idx in range(total_batches):
            # 取当前批次的实体对
            start_idx = batch_idx * batch_size
            end_idx = min(start_idx + batch_size, len(sampled_pairs))
            batch_pairs = sampled_pairs[start_idx:end_idx]

            # 生成批次标识用于进度跟踪
            batch_keys = [generate_pair_key(p, args.subtype) for p in batch_pairs]

            # 检查是否全部已完成
            if args.resume:
                all_completed = all(progress_manager.is_completed(k) for k in batch_keys)
                if all_completed:
                    print(f"[批次 {batch_idx+1}/{total_batches}] 跳过已完成批次")
                    continue

            print(f"\n[批次 {batch_idx+1}/{total_batches}] 处理 {len(batch_pairs)} 对实体...")

            progress_manager.stats['total_attempted'] += len(batch_pairs)

            # 构建批量提示词
            prompt = build_batch_prompt(batch_pairs, args.subtype, success_count + 1)

            # 调用API
            response = glm_client.generate(prompt)
            if not response:
                print(f"  API调用失败")
                for pair_key in batch_keys:
                    progress_manager.mark_failed(pair_key, "API调用失败")
                continue

            # 解析批量响应
            records = parse_batch_json(response)
            if not records:
                print(f"  批量JSON解析失败")
                for pair_key in batch_keys:
                    progress_manager.mark_failed(pair_key, "批量JSON解析失败")
                continue

            # 处理每条记录
            for j, data in enumerate(records):
                if j >= len(batch_pairs):
                    print(f"  返回记录数({len(records)})超过输入数({len(batch_pairs)})，跳过多余记录")
                    break

                pair = batch_pairs[j]
                pair_key = batch_keys[j]
                ...
                print(f"  成功: {data['id']} - {data['question'][:25]}...")

            # 定期保存进度
            if (batch_idx + 1) % max(1, args.checkpoint_interval // batch_size) == 0:
