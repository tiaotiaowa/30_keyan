# -*- coding: utf-8 -*-
"""
LoRA微调模型评测脚本 - Prompt Template格式
==========================================

使用与exp0一致的格式：添加详细的prompt_template
...
使用方法:
    python evaluate_prompt.py \
        ...
        --output-dir ./results/prompt_format
...
# Prompt模板 - 与exp0/generation_config.yaml一致
...
def generate_single(model, tokenizer, question: str) -> str:
    """
    单条推理 - 使用prompt_template格式（与exp0一致）
...
def main():
    parser = argparse.ArgumentParser(description="LoRA模型评测 - Prompt Template格式")
    ...
    parser.add_argument("--output-dir", type=str, default="./results/prompt_format", help="输出目录")
    ...
    # 打印摘要
    print("\n" + "="*50)
    print("评测结果摘要 (Prompt Template格式)")
    print("="*50)
