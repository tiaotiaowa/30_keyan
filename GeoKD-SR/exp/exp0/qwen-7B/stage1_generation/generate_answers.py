"""
Stage 1: 答案生成脚本 (Qwen2.5-7B-Instruct)
使用 Qwen2.5-7B-Instruct 模型生成测试集的预测答案
"""
import argparse
import json
import logging
...
class AnswerGenerator:
    ...
    def run(self, input_file: str = None, output_file: str = None):
        """
        运行答案生成

        Args:
            input_file: 输入文件路径（可选，覆盖配置）
            output_file: 输出文件路径（可选，覆盖配置）
        """
        # 加载模型
        self.load_model()
        ...
        logger.info(f"开始生成答案，共 {len(test_data)} 条...")
...
def main():
    parser = argparse.ArgumentParser(description='Stage 1: 答案生成 (Qwen2.5-7B)')
    parser.add_argument(
        '--config',
        type=str,
    ...
    generator.run(input_file=args.input, output_file=args.output)
