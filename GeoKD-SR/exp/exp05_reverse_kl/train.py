...
def main():
    parser = argparse.ArgumentParser(description="Exp5: B2+C3（逆向KL蒸馏）训练")
    ...
    print("=" * 60)
    print("Exp5: B2+C3（逆向KL蒸馏）")
    print("=" * 60)
    print(f"配置文件: {args.config}")
    print(f"教师模型: {config['model']['teacher']['name']} (4-bit量化)")
    print(f"学生模型: {config['model']['student']['name']}")
    print(f"Forward KL权重: {config['distillation']['reverse_kl']['forward_weight']}")
    print(f"Reverse KL权重: {config['distillation']['reverse_kl']['reverse_weight']}")
    print("=" * 60)
    ...
    trainer = ReverseKLDistillationTrainer(
        ...
        forward_weight=config['distillation']['reverse_kl']['forward_weight'],
        reverse_weight=config['distillation']['reverse_kl']['reverse_weight'],
        model=student_model,
        args=training_args,
