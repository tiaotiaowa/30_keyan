...
def main():
    parser = argparse.ArgumentParser(description="Exp3: B2+C1(Learnable) 评估")
    ...
    # 加载学习到的权重
    weights_path = Path(args.checkpoint) / "learned_weights.json"
    if weights_path.exists():
        with open(weights_path, "r") as f:
            learned_weights = json.load(f)
        print(f"学习到的权重: {learned_weights}")
    ...
    summary = {
        "experiment": "Exp3: B2+C1(Learnable)",
